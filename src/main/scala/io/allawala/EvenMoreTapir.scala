...
import sttp.tapir.ztapir.{ZPartialServerEndpoint, ZServerEndpointInParts}
import sttp.tapir.{Tapir, TapirAliases}
import zio.{RIO, ZIO}

object EvenMoreTapir
    extends Tapir
    with ZHttp4sServerInterpreter
    with TapirJsonCirce
    with TapirAliases
  ...
  // this is copied from the package object sttp.tapir.ztapir._
  // I noticed this is not a trait - I'll fix it in the next release so that you could mix it into the object :)
  ...
  implicit class RichZEndpoint[I, E, O](e: ZEndpoint[I, E, O]) {
    ...
    def zServerLogicForCurrent[R, U](f: I => ZIO[R, E, U]): ZPartialServerEndpoint[R, U, Unit, E, O] =
      new ZPartialServerEndpoint[R, U, Unit, E, O](
        e.copy(input = EndpointIO.Empty(Codec.idPlain(), EndpointIO.Info.empty))
      ) {
        override type T = I
